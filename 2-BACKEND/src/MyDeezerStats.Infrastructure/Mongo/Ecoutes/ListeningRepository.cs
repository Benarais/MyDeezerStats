...
namespace MyDeezerStats.Infrastructure.Mongo.Ecoutes
{
    public class ListeningRepository : IListeningRepository
    {
        ...
        /*public async Task<AlbumListening?> GetAlbumsWithAsync(string title, string artist, DateTime? from = null, DateTime? to = null)
        {
            ...
            var pipeline = new[]
            {
        // Étape 1: Filtrer les écoutes de l'album/artiste spécifique
        PipelineStageDefinitionBuilder.Match(filter),

        // Étape 2: Normaliser les noms de pistes en retirant les featurings (ex: "feat.", "ft.", etc.)
        new BsonDocument("$addFields",
            new BsonDocument("NormalizedTrack",
                new BsonDocument("$trim",
                    new BsonDocument("input",
                        new BsonDocument("$replaceAll",
                            new BsonDocument("input", "$Track")
                            .Add("find", @"\s*(feat\.|ft\..*|feat.*|ft.*)")  // Supprime 'feat.' ou variations
                            .Add("replacement", "")))))),

        // Étape 3: Grouper par piste normalisée
        new BsonDocument("$group",
            new BsonDocument
            {
                { "_id", "$NormalizedTrack" },  // Utilise le titre normalisé de la piste
                { "Count", new BsonDocument("$sum", 1) },
                { "Album", new BsonDocument("$first", "$Album") },
                { "Artist", new BsonDocument("$first", "$Artist") }
            }),
        ...
        /* public async Task<AlbumListening?> GetAlbumsWithAsync(string title, string artist, DateTime? from = null, DateTime? to = null)
         ...
         Builders<BsonDocument>.Filter.Or(
             ...
             Builders<BsonDocument>.Filter.Regex("Artist", new BsonRegularExpression($"^{Regex.Escape(artist)}$", "i")),
             // Contient l'artiste comme partie du nom
             Builders<BsonDocument>.Filter.Regex("Artist", new BsonRegularExpression($"{Regex.Escape(artist)}", "i")),
             // Gestion des alias (ex: "Caballero & JeanJass" vs "Caballero and JeanJass")
             Builders<BsonDocument>.Filter.Regex("Artist", new BsonRegularExpression($"{Regex.Escape(artist.Replace("&", "and"))}", "i")),
             Builders<BsonDocument>.Filter.Regex("Artist", new BsonRegularExpression($"{Regex.Escape(artist.Replace(" and ", " & "))}", "i"))
         ),
         Builders<BsonDocument>.Filter.Exists("Track"),
         Builders<BsonDocument>.Filter.Ne("Track", "")
             ...
             // Étape 1: Filtrer les écoutes de l'album/artiste spécifique
              ...
              // Étape 2: Normaliser les noms de pistes
             new BsonDocument("$addFields",
                 new BsonDocument("NormalizedTrack",
                     new BsonDocument("$trim",
                         new BsonDocument("input", "$Track")))),
